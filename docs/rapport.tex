...
% Logo / titre banque
{\fontsize{48}{52}\selectfont\bfseries\textcolor{primary-blue}{Captus}\textcolor{white}{Bank}}

\vspace{0.3cm}
{\large\textcolor{gray!60}{Application bancaire de démonstration}}

\vspace{1.5cm}
\textcolor{primary-blue}{\rule{10cm}{1.5pt}}

\vspace{0.8cm}
{\fontsize{22}{26}\selectfont\bfseries Rapport Technique}\\[0.4cm]
{\fontsize{16}{20}\selectfont Session Fixation, XSS \& CSRF}\\[0.2cm]
{\large Démonstration et contre-mesures}
...
% Badges vulnérabilités
\begin{tcolorbox}[
  enhanced, width=9cm,
  colback=dark-bg!80!black, colframe=vuln-red,
  arc=8pt, halign=center,
  top=10pt, bottom=10pt
]
  \textcolor{vuln-red}{\large\faExclamationTriangle}\\[4pt]
  {\normalsize\textcolor{white}{CWE-384 · CWE-79 · CWE-352}}\\[2pt]
  {\small\textcolor{gray!60}{CWE-256 · CWE-1004 · CWE-200}}
\end{tcolorbox}

\vspace{1.5cm}

% Équipe
{\large\bfseries Équipe}\\[0.5cm]
\begin{tcolorbox}[
  enhanced, width=9cm,
  colback=dark-bg!80!black, colframe=primary-blue!60,
  arc=8pt,
  top=12pt, bottom=12pt
]
  \centering
  {\normalsize Mohamed \textbf{MOHSSINE}}\\[4pt]
  {\normalsize Ali \textbf{Braiki}}\\[4pt]
  {\normalsize Youssef \textbf{Benaouda}}\\[4pt]
  {\normalsize Grif \textbf{Omar}}
\end{tcolorbox}

\vspace{1.5cm}

{\large Cours d'Ethical Hacking}\\[0.2cm]
{\normalsize\textcolor{gray!60}{Université — Projet P03}}\\[0.4cm]
{\normalsize\textcolor{gray!60}{Mai 2026}}
...
\begin{tabular}{@{}llll@{}}
\toprule
\textbf{\#} & \textbf{Vulnérabilité} & \textbf{OWASP Top 10} & \textbf{CWE} \\
\midrule
1 & Session Fixation & A07 — Identification Failures & CWE-384 \\
2 & XSS Stocké & A03 — Injection & CWE-79 \\
3 & XSS Réfléchi & A03 — Injection & CWE-79 \\
4 & CSRF & A01 — Broken Access Control & CWE-352 \\
5 & Mots de passe en clair & A02 — Cryptographic Failures & CWE-256 \\
6 & Cookies non sécurisés & A05 — Security Misconfiguration & CWE-1004 \\
7 & Information Disclosure & A05 — Security Misconfiguration & CWE-200 \\
\bottomrule
\end{tabular}
...
\texttt{attacker-server} & \texttt{evil.attacker.lab} & 8080 & C2 attaquant : dashboard + collecte de sessions \\
...
\section{Session Fixation — \cwe{384}}
...
% -------- 3.2 XSS STOCKÉ --------
\section{Cross-Site Scripting Stocké — \cwe{79}}
...
% -------- 3.3 XSS RÉFLÉCHI --------
\section{Cross-Site Scripting Réfléchi — \cwe{79}}
...
% -------- 3.4 CSRF --------
\section{Cross-Site Request Forgery — \cwe{352}}
...
\section{Stockage des mots de passe en clair — \cwe{256}}
...
% -------- 3.6 COOKIES --------
\section{Cookies non sécurisés — \cwe{1004}}
...
\subsection*{L'app sécurisée résiste}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\linewidth]{photos/secure-csrf-bloque.png}
\caption{App sécurisée — requête CSRF rejetée (token manquant ou invalide)}
\end{figure}
...
\endhead
Session Fixation &
  \xmark\ \texttt{use\_strict\_mode=0} \newline
  \xmark\ \texttt{use\_only\_cookies=0} \newline
  \xmark\ Pas de \texttt{session\_regenerate\_id()} &
  \cmark\ \texttt{use\_strict\_mode=1} \newline
  \cmark\ \texttt{use\_only\_cookies=1} \newline
  \cmark\ \texttt{session\_regenerate\_id(true)} au login \\[6pt]

XSS Stocké/Réfléchi &
  \xmark\ \texttt{<?= \$r['content'] ?>} \newline
  \xmark\ Pas de CSP &
  \cmark\ \texttt{htmlspecialchars()} sur toutes les sorties \newline
  \cmark\ CSP stricte (\texttt{script-src 'self'}) \\[6pt]

CSRF &
  \xmark\ Pas de token CSRF \newline
  \xmark\ Accepte les paramètres GET &
  \cmark\ Token CSRF par session (\texttt{random\_bytes(32)}) \newline
  \cmark\ POST uniquement \\[6pt]

Mots de passe &
  \xmark\ Stockés en clair &
  \cmark\ bcrypt (coût 12) + \texttt{password\_verify()} \\[6pt]

Cookies &
  \xmark\ Pas de HttpOnly \newline
  \xmark\ Pas de Secure \newline
  \xmark\ Pas de SameSite &
  \cmark\ \texttt{HttpOnly=true} \newline
  \cmark\ \texttt{Secure=true} \newline
  \cmark\ \texttt{SameSite=Strict} \\[6pt]

Information Disclosure &
  \xmark\ \texttt{X-Powered-By} présent \newline
  \xmark\ SID affiché dans le HTML &
  \cmark\ \texttt{expose\_php=Off} \newline
  \cmark\ Aucun SID exposé \\[6pt]

Détection d'intrusion &
  \xmark\ Aucun logging &
  \cmark\ Audit log structuré \newline
  \cmark\ Fingerprint de session \newline
  \cmark\ Compte honeytoken \\

\bottomrule
\caption{Tableau comparatif des mesures de sécurité}
\end{longtable}

\section{Visualisation de l'app sécurisée}

\begin{figure}[H]
\centering
\begin{subfigure}[b]{0.48\linewidth}
  \includegraphics[width=\linewidth]{photos/secure-session-regeneree.png}
  \caption{Nouveau SID après login (session\_regenerate\_id)}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\linewidth}
  \includegraphics[width=\linewidth]{photos/secure-xss-bloque.png}
  \caption{Payload XSS affiché comme texte (htmlspecialchars)}
\end{subfigure}
\caption{App sécurisée — les attaques sont neutralisées}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\linewidth]{photos/csp-header.png}
\caption{En-tête Content-Security-Policy dans les réponses de l'app sécurisée}
\end{figure}
...
\section{Bilan des vulnérabilités démontrées}

Ce projet a permis de démontrer concrètement que la sécurité d'une application web ne dépend pas d'une seule mesure, mais d'une \textbf{défense en profondeur} combinant plusieurs protections indépendantes. Les sept vulnérabilités présentées illustrent une réalité fréquente dans les applications réelles :

\begin{itemize}
  \item Des vulnérabilités qui semblent mineures prises isolément (SID dans le HTML, GET sur un formulaire de virement) deviennent \textbf{critiques} lorsqu'elles sont enchaînées
  \item Une seule vulnérabilité XSS suffit à contourner une protection anti-CSRF si les cookies ne sont pas \texttt{HttpOnly}
  \item La session fixation est indétectable par la victime — elle ne voit qu'un rechargement de page ou un message \og session expirée \fg
\end{itemize}

\section{Impact réel de la chaîne d'attaque}

L'enchaînement des failles démontrées dans ce projet constitue une attaque complète de bout en bout :

\begin{enumerate}
  \item Mallory dispose d'un compte légitimement ouvert sur la banque (accès blanc)
  \item Mallory injecte un payload XSS via les commentaires (\textbf{surface publique})
  \item Alice visite le site normalement — son cookie est remplacé (\textbf{sans clic suspect})
  \item Alice se reconnecte — Mallory hérite de sa session (\textbf{hijack complet})
  \item Mallory vide le compte d'Alice (\textbf{impact financier direct})
\end{enumerate}

\textbf{Impact estimé} : vol de 12\,450~€, violation du RGPD, atteinte à la réputation de la banque, potentielle mise en cause de la responsabilité légale.

\section{Leçons retenues}

\begin{enumerate}
  \item \textbf{Toujours appeler \texttt{session\_regenerate\_id(true)} après une authentification réussie} — c'est une ligne de code qui empêche une classe entière d'attaques
  \item \textbf{Échapper toutes les données dynamiques avant affichage} — aucune exception, même pour des données \og internes \fg
  \item \textbf{Valider l'origine des requêtes sensibles} — token CSRF + \texttt{SameSite=Strict}
  \item \textbf{La sécurité n'est pas ajoutée en fin de projet} — elle doit être intégrée dès la conception
\end{enumerate}

\section{Perspectives}

Les contre-mesures implémentées dans la version sécurisée constituent une base solide. Des améliorations supplémentaires pourraient inclure :

\begin{itemize}
  \item Authentification multi-facteurs (TOTP/FIDO2)
  \item Rate limiting et protection anti-brute-force
  \item Web Application Firewall (WAF) en amont de Nginx
  \item Chiffrement de la colonne de données sensibles en base
  \item Audit et monitoring SIEM avec alertes en temps réel
\end{itemize}
